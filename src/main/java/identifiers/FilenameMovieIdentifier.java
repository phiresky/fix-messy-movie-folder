...
public abstract class FilenameMovieIdentifier extends MovieIdentifier {
	...
	public FilenameMovieIdentifier(String sourcename,
			Function<String, Optional<ImdbId>> searchfn) {
	...
	public MovieInfo tryIdentifyMovie(Path input) {
		...
		if (info == null) {
			System.out.println(sourcename + ": Could not find "
					+ input.getFileName() + ":\n" + log);
		} else {
			info.setPath(input);
		}
	...
	public MovieInfo tryIdentifyMovie(String search) {
		ImdbId imdbid = searchcache.getCached(search,
				() -> searchfn.apply(search).orElse(null));
